export default function MarketplaceAuctions() {
    return (
        <div id="marketplace-auctions" className="py-16 w-full">
            <div className="flex items-center py-3 justify-between border-2 border-x-0 border-black">
                <span className="text-2xl font-body font-semibold">Explore marketplace</span>
                <div id="btn" className="border p-2 rounded-full flex items-center justify-center w-[56px] h-[57px]
                scale-animation cursor-pointer ml-auto mr-4">
                    <svg width="34" height="22" viewBox="0 0 34 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M0 11C0 10.6759 0.127925 10.3651 0.355633 10.1359C0.583342 9.9067 0.89218 9.77795 1.21421 9.77795H29.8525L22.2103 2.08881C21.9823 1.85934 21.8542 1.54811 21.8542 1.22359C21.8542 0.899077 21.9823 0.587851 22.2103 0.358382C22.4383 0.128914 22.7475 7.64589e-09 23.07 0C23.3924 -7.64589e-09 23.7016 0.128914 23.9296 0.358382L33.6433 10.1348C33.7564 10.2483 33.8461 10.3832 33.9073 10.5316C33.9685 10.6801 34 10.8393 34 11C34 11.1607 33.9685 11.3199 33.9073 11.4684C33.8461 11.6168 33.7564 11.7517 33.6433 11.8652L23.9296 21.6416C23.7016 21.8711 23.3924 22 23.07 22C22.7475 22 22.4383 21.8711 22.2103 21.6416C21.9823 21.4121 21.8542 21.1009 21.8542 20.7764C21.8542 20.4519 21.9823 20.1407 22.2103 19.9112L29.8525 12.2221H1.21421C0.89218 12.2221 0.583342 12.0933 0.355633 11.8641C0.127925 11.6349 0 11.3241 0 11Z" fill="#616161"/>
                    </svg>
                </div>
            </div>
            <div className="flex items-center py-3 border-2 border-x-0 border-t-0 border-black">
                <span className="text-2xl font-body font-semibold">See auctions</span>
                <div id="btn" className="border p-2 rounded-full flex items-center justify-center w-[56px] h-[57px]
                scale-animation cursor-pointer ml-auto mr-4">
                    <svg width="34" height="22" viewBox="0 0 34 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M0 11C0 10.6759 0.127925 10.3651 0.355633 10.1359C0.583342 9.9067 0.89218 9.77795 1.21421 9.77795H29.8525L22.2103 2.08881C21.9823 1.85934 21.8542 1.54811 21.8542 1.22359C21.8542 0.899077 21.9823 0.587851 22.2103 0.358382C22.4383 0.128914 22.7475 7.64589e-09 23.07 0C23.3924 -7.64589e-09 23.7016 0.128914 23.9296 0.358382L33.6433 10.1348C33.7564 10.2483 33.8461 10.3832 33.9073 10.5316C33.9685 10.6801 34 10.8393 34 11C34 11.1607 33.9685 11.3199 33.9073 11.4684C33.8461 11.6168 33.7564 11.7517 33.6433 11.8652L23.9296 21.6416C23.7016 21.8711 23.3924 22 23.07 22C22.7475 22 22.4383 21.8711 22.2103 21.6416C21.9823 21.4121 21.8542 21.1009 21.8542 20.7764C21.8542 20.4519 21.9823 20.1407 22.2103 19.9112L29.8525 12.2221H1.21421C0.89218 12.2221 0.583342 12.0933 0.355633 11.8641C0.127925 11.6349 0 11.3241 0 11Z" fill="#616161"/>
                    </svg>
                </div>
            </div>
        </div>
    )
}